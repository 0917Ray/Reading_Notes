# bigram4GPT_v2.py
# A minimal implementation of a bigram language model including softmax using PyTorch 
...
# One head self-attention module
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        ...
        self.proj = nn.Linear(embed_dim, embed_dim) # final projection layer
        self.dropout = nn.Dropout(dropout)
...
# Block: communication followed by computation (feed-forward)
class Block(nn.Module):
    ...
    def forward(self, x):
        x = self.ln1(x + self.sa(x))   # residual connection with layer norm
        x = self.ln2(x + self.ffwd(x)) # residual connection with layer norm
        return x
...
model = BiGramLanguageModel(embed_dim)
m = model.to(device)
...
save_path = os.path.join(base_dir, 'results', 'training_loss_one_attention.pdf')
