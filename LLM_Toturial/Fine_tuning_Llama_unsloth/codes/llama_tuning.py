...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        per_device_train_batch_size = 16, # Number of parallel data per GPU
        gradient_accumulation_steps = 16, # Effective batch size = per_device_train_batch_size * gradient_accumulation_steps
        warmup_steps = 50,
        ...
        optim = "adamw_8bit",
        weight_decay = 0.01,
        lr_scheduler_type = "cosine", # "consine", "linear" or etc.
        seed = 3407,
        eval_strategy = "steps",
...
# 6. Save Model
model.save_pretrained(f"{current_dir}/lora_model_{run_id}")
tokenizer.save_pretrained(f"{current_dir}/lora_model_{run_id}")